...
export const dataToDb = (datasets) => new Promise((resolve, reject) => {
  ...
  async.each(
    datasets,
    (dataset, callback) => {
      const reportInfo = dataset.data.Report.table3[0].Detail_Collection[0].Detail[0].$;
      ...
      client.query(
        ...
        (err) => {
          ...
          const data = dataset.data.Report.BillingDetail[0].Detail_Collection[0].Detail;
          ...
          async.each(
            ...
            (err2) => {
              ...
              const message = `Processed ${dataset.fileName} with ${items.length} records`;
              console.log(message);
